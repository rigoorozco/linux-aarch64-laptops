...
static int ti_sn65dsi86_backlight_update(struct ti_sn65dsi86 *pdata);
...
static int __maybe_unused ti_sn65dsi86_resume(struct device *dev)
{
	...
	return ti_sn65dsi86_backlight_update(pdata);
}
...
static int ti_sn65dsi86_backlight_update(struct ti_sn65dsi86 *pdata)
{
	...
	ti_sn65dsi86_write_u16(pdata, SN_BACKLIGHT_SCALE_LOW, pdata->max_brightness);

	if (pdata->brightness) {
		/* Set brightness */
		ti_sn65dsi86_write_u16(pdata, SN_BACKLIGHT_LOW, pdata->brightness);
...
{
	struct ti_sn65dsi86 *pdata = bl_get_data(bl);
...
static int ti_sn_backlight_init(struct ti_sn65dsi86 *pdata)
{
